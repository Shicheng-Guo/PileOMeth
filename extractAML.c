...
uint32_t sum_nmethyl = 0;
uint32_t sum_nunmethyl = 0;
int flag = 1;
int flag2 = 0;
...
typedef struct{
  ...
  Config *config;
  char *chrom;
  int32_t pos, width, nmethyl, nunmethyl;
  char base;
}Buf;
*/
void writeCall(FILE *of, Config *config, char *chrom, int32_t pos, int32_t width, uint32_t nmethyl, uint32_t nunmethyl, char base) { 
   ...
   // (flag == 1 ? printf("%s:%i-", chrom, pos) : printf("")); flag = 0;
    

    if (!config->fraction && !config->logit && !config->counts && !config->methylKit) {
        //(flag2 == 1 ? printf("%i\t", pos+width) : printf("")); //HIW
        fprintf(of, \
        ...
        //(flag2 == 1 ? printf("%i\t", pos+1) : printf("")); //HIW
        fprintf(of, \
            "%s.%i\t%s\t%i\t%c\t%i\t%f\t%f\n", \
            chrom, \
            pos+1, \
...
//Vector v;
//vector_init(&v);
Buf array[1000] = {};  //PROBLEM WITH STATIC MEMORY ALLOCATION
int size = 0;
int indx = 0;

void buffer(FILE *of, Config *config, char *chrom, int32_t pos, int32_t width, uint32_t nmethyl, uint32_t nunmethyl, char base, int flag) { 
  size++;
    array[indx].of = of;
    array[indx].config = config;
    array[indx].chrom = chrom;
    array[indx].pos = pos;
    array[indx].width = width;
    array[indx].nmethyl = nmethyl;
    array[indx].nunmethyl = nunmethyl;
    array[indx].base = base;
    indx++;

  if(!flag){
    printf("%s%i\n", "sum_nmethyl: ", sum_nmethyl);
    printf("%s%i\n", "sum_numethyl: ", sum_nunmethyl);
    for(int i = 0; i < size; i++){
      writeCall(array[i].of, array[i].config, array[i].chrom, array[i].pos, \
      1, array[i].nmethyl, array[i].nunmethyl, array[i].base);
    }
...
void extractCalls(Config *config) {
    ...
    int idxBED = 0, strand;
    uint32_t nmethyl = 0, nunmethyl = 0;    //init variables here (HIW)
	...
	if (config->reg){
        ...
        //(HIW)
        sum_nmethyl += nmethyl;
        sum_nunmethyl += nunmethyl;  

        (cust_mplp_auto(iter, &tid, &pos, &n_plp, plp) <= 0 ? (flag2 = 1) : (flag2 = 0));  
        
        if(!config->merge || type==2) { 
            //printf("%s:%i-", hdr->target_name[tid], pos);
            //printf("%i\n", pos);
            //
            //buffer(config->output_fp[type], config, hdr->target_name[tid], pos, 1, nmethyl, nunmethyl, base, ret);
...
int extract_main(int argc, char *argv[]) {
    ...
    printf("%i\t%i\t", sum_nmethyl, sum_nunmethyl);
    double total = (double)(sum_nmethyl + sum_nunmethyl);

    printf("%f\n", (double)(sum_nmethyl/total));
    //printf("%f\n", (double)(sum_nunmethyl/total));
